...
We must formally establish the architecture, boundaries, security posture, atomic persistence mechanics, stable ID invariants, dependency selections, single SQLite transaction boundaries, and consistency models between the filesystem Markdown vault and the encrypted SQLite storage subsystem.
...
### 2. Stable Document ID Invariant (UUID v4)
Document identity is strictly grounded in a stable UUID v4 document ID:
- **Creation & Validation**: When a document is first created, a stable UUID v4 ID is generated. Parsing existing Markdown documents enforces that `metadata.id` parses as a valid UUID and has version `uuid::Version::Random` (v4). Invalid IDs or non-v4 UUIDs are rejected with `VaultError::MalformedFrontmatter`. Auto-generating replacement IDs on parse is forbidden.
...
- **Immutable Primary Key**: Updates modify metadata fields (`title`, `size_bytes`, `content_hash`, `schema_version`, `updated_at`) without re-assigning the primary key `id`.
...
### 5. Single SQLite Transaction Boundary (Metadata + Audit Log)
To guarantee consistency across SQLite tables:
- **Atomic SQLite Transaction**: Metadata index mutations (`vault_documents`) and corresponding audit log entries (`system_audit_logs`) are executed and committed inside **ONE single immediate SQLite transaction** (`log_audit_event_tx`).
- **Rollback Guarantee**: If audit log insertion fails, the SQLite metadata mutation rolls back completely, ensuring metadata is never left committed without an audit record.
- **No Global Multi-Subsystem ACID Transaction**: Life OS does not claim native multi-resource ACID atomicity spanning both the OS filesystem and SQLite simultaneously.
- **Mutation Sequence**: The physical Markdown file is written atomically first, followed by the single SQLite transaction updating `vault_documents` and `system_audit_logs`.
- **Database Error Propagation**: Queries checking for existing metadata explicitly distinguish `VaultError::NotFound` (which enters the create path) from database errors (which are immediately propagated rather than swallowed).
...
## Consequences
- **Positive**: Total user data sovereignty, strict UUID v4 stable document identity guarantees, atomic SQLite metadata/audit commitments, zero proprietary lock-in, atomic file writes preventing partial corruption, and auditable metadata tracking.
